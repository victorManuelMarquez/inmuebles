...
<body class="d-flex flex-column vh-100 bg-body-secondary" style="padding-top: 56px;">
    ...
    <main class="flex-shrink-0 py-5">
        <!--<%=(new EmployeeDAOImpl().getById(Integer.parseInt(request.getParameter("id"))))%>-->
        <%
        try {
            Employee employee;
            employee = (new EmployeeDAOImpl()).getById(Integer.parseInt(request.getParameter("id")));
        %>
        <div class="container" id="apartadoIngresar">
            <div class="shadow bg-body px-3 py-3 p-lg-5">
                <h3 class="mb-3">Datos:</h3>
                <form action="UpdateEmpleado" method="POST" class="needs-validation">
                    <input class="d-none" type="text" name="idGerente" value='<%=request.getParameter("manager")%>'>
                    <input class="d-none" type="text" name="idEmpleado" value="<%=employee.getId()%>">
                    <div class="input-group mb-3">
                        <span class="input-group-text">DNI:</span>
                        <input type="text" class="form-control" name="nuevoDni" value="<%=employee.getPersonId()%>" placeholder="DNI sin puntos ni guiones" required>
                    </div>
                    <div class="input-group mb-3">
                        <span class="input-group-text">Nombre Completo:</span>
                        <input type="text" class="form-control" name="nuevoNombre" value="<%=employee.getFullName()%>" placeholder="Nombre completo" required>
                    </div>
                    <div class="input-group mb-3">
                        <span class="input-group-text">Domicilio:</span>
                        <input type="text" class="form-control" name="nuevoDomicilio" value="<%=employee.getAddress()%>" placeholder="Dirección donde reside" required>
                    </div>
                    <div class="form-check mb-3">
                        <input id="checkBox" type="checkbox" class="form-check-input" name="nuevoEstado" value="" <%=employee.isActive() ? "checked" : ""%>>
